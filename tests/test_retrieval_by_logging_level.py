...
def test_retrieval_by_logging_level(users_and_logger: list) -> None:
    ...
    required_logging_level: LoggingLevel = LoggingLevel.CRITICAL
    retrieved_log, *remaining_logs = logger.retrieve_by_logging_level(
        required_logging_level)

    assert len(remaining_logs) == 0

    assert log_of_Alexey == retrieved_log
